...
module.exports = class FavoriteCampground {
  ...
  static async createFavorite({ user_email, facility_id, facility_name, facility_description, facility_directions, facility_phone, facility_email, reservable, images }){
    ...
    // I'd opt for a map here and write this more declaritively
    const arrOfImages = [];
  ...
  static async deleteFavorite(id){
    const { rows } = await pool.query(
      'DELETE FROM favorites WHERE facility_id = $1 RETURNING *', [id]
    );


    const images = await pool.query(
      'DELETE FROM images WHERE favorites_facility_id = $1 RETURNING *', [id]
    );
    // console.log('IMAGES', images);
    return { ...rows, images: images.rows };
